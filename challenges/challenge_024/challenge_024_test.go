...
import (
	...
	challenge "code-challenges-go/challenges/challenge_024"

	"github.com/stretchr/testify/require"
)

func TestChallenge_024(t *testing.T) {
	t.Parallel()
	...
	successCases := []struct {
	...
	}{
		{
			input: input{
				N:          2,
				operations: []string{"VALUE 3 _", "ADD $0 4"},
			},
			output: []int{3, 7},
		},
		{
			input:  input{N: 3, operations: []string{"VALUE 20 _", "ADD $0 100", "ADD $1 1"}},
			output: []int{20, 120, 121},
		},
		{
			input:  input{N: 2, operations: []string{"VALUE 12 _", "SUB $0 3"}},
			output: []int{12, 9},
		},
		{
			input:  input{N: 2, operations: []string{"VALUE 4 _", "MULT 4 $0"}},
			output: []int{4, 16},
		},
		{
			input:  input{N: 3, operations: []string{"ADD 1 2", "SUB 3 1", "MULT 2 4"}},
			output: []int{3, 2, 8},
		},
		{
			input: input{
		...
		},
		{
			...
			output: []int{0, 1, 1, 2, 3, 5, 8, 13, 21},
		},
		{
			input:  input{N: 2, operations: []string{"ADD $1 20", "VALUE 32 _"}},
			output: []int{52, 32},
		},
		{
			input: input{
				N:          4,
				operations: []string{"SUB $1 4", "VALUE 3 _", "ADD 8 $1", "MULT $0 $2"},
			},
			output: []int{-1, 3, 11, -11},
		},
		{
			input: input{
				N: 6,
				operations: []string{"MULT $5 $2",
					"ADD $5 $0",
					"VALUE 12 _",
					"ADD $2 $2",
					"MULT $3 $2",
			...
			output: []int{144, 156, 12, 24, 288, 12},
		},
		{
			// prettier-ignore
			input: input{N: 100, operations: []string{
		...
		{
			// prettier-ignore
			input: input{N: 92, operations: []string{
	...
	failureCases := []struct {
		...
		},
		{
			input: input{
				N:          2,
				operations: []string{"VALUE 3 _", "ADD $0 4", "ADD $0 4"},
			},
			err: errors.New(challenge.INVALID_OPERATION),
		},
	}
	...
	for i := range successCases {
		...
		t.Run(fmt.Sprintf("success case %d", i), func(t *testing.T) {
			...
			result, err := challenge.Solution(tc.input.N, tc.input.operations)
