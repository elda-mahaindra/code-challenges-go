...
func isValid(side, diameter float64) error {
	switch {
	case diameter <= 0 || diameter >= side:
		return errors.New(OUT_OF_RANGE_DIAMETER)
	case side <= diameter || side >= 100:
		return errors.New(OUT_OF_RANGE_SIDE)
	default:
		return nil
	}
}

func Solution(side, diameter float64) (int, error) {
	err := isValid(side, diameter)
	if err != nil {
		return -1, err
	}
	...
	doughSquarePerBiscuit := math.Pow(diameter, 2)
	biscuitCircle := (math.Pi / 4) * math.Pow(diameter, 2)

	remainingDoughSquare := math.Pow(side, 2)

	for remainingDoughSquare >= doughSquarePerBiscuit {
		currentSide := math.Pow(remainingDoughSquare, 0.5)

		made := math.Pow(math.Floor(currentSide/diameter), 2)

		if wastefulBakerMade < 0 {
			wastefulBakerMade += int(made) + 1
			frugalBakerMade += int(made) + 1
		} else {
			frugalBakerMade += int(made)
		}

		remainingDoughSquare -= made * biscuitCircle
	}

	return int(frugalBakerMade - wastefulBakerMade), nil
}
