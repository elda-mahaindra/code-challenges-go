...
import (
	...
	challenge "code-challenges-go/challenges/challenge_008"

	"github.com/stretchr/testify/require"
)

func TestChallenge_008(t *testing.T) {
	t.Parallel()
	...
	successCases := []struct {
		...
		{
			input:  input{N: 8, playerSigns: []string{"4 R", "1 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
			output: "2 6 5 1",
		},
		{
			input:  input{N: 2, playerSigns: []string{"1 S", "2 S"}},
			output: "1 2",
		},
		{
			input: input{
	...
	failureCases := []struct {
		...
		{
			input: input{N: 1, playerSigns: []string{"4 R", "1 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
			err:   errors.New(challenge.OUT_OF_RANGE_N),
		},
		{
			input: input{N: 12, playerSigns: []string{"4 R", "1 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
			err:   errors.New(challenge.INVALID_N),
		},
		{
			input: input{N: 16, playerSigns: []string{"4 R", "1 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
			err:   errors.New(challenge.INVALID_PLAYER_SIGNS),
		},
		{
			input: input{N: 8, playerSigns: []string{"$ R", "1 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
			err:   errors.New(challenge.INVALID_PLAYER_SIGNS),
		},
		{
			input: input{N: 8, playerSigns: []string{"-4 R", "1 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
		...
		{
			input: input{N: 8, playerSigns: []string{"4 R", "4 P", "8 P", "3 R", "7 C", "5 S", "6 L", "2 L"}},
			err:   errors.New(challenge.INVALID_PLAYER_SIGNS),
		},
	}
	...
	for i := range successCases {
		...
		t.Run(fmt.Sprintf("success case %d", i), func(t *testing.T) {
			...
			result, err := challenge.Solution(tc.input.N, tc.input.playerSigns)
