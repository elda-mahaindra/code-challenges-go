...
import (
	...
	challenge "code-challenges-go/challenges/challenge_033"

	"github.com/stretchr/testify/require"
)

func TestChallenge_033(t *testing.T) {
	t.Parallel()
	...
	successCases := []struct {
		...
		},
		{
			input: input{
				n:           1,
				assignments: []string{"ARR[-5..-3] = 11 22 33"},
				element:     "ARR[-4]",
			},
			output: 22,
		},
		{
			input: input{
				n:           1,
				assignments: []string{"X[0..3] = 1 3 3 7"},
				element:     "X[X[2]]",
			},
			output: 7,
		},
		...
		},
		{
			input: input{
				...
				element: "B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[B[52]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]]",
			},
			output: 28,
		},
		{
			input: input{
				...
				element: "V[S[W[Q[S[P[M[C[A[R[B[Z[V[S[Q[V[V[J[W[-17]]]]]]]]]]]]]]]]]]]",
			},
			output: -2,
		},
		{
	...
	failureCases := []struct {
		...
		},
		{
			...
			err: errors.New(challenge.INVALID_ARRAY_ASSIGNMENTS),
		},
		{
			input: input{
				n: 3,
		...
		},
		{
			...
			err: errors.New(challenge.INVALID_ELEMENT),
		},
		{
			input: input{
				n: 3,
	...
	for i := range successCases {
		...
		t.Run(fmt.Sprintf("success case %d", i), func(t *testing.T) {
			...
			result, err := challenge.Solution(tc.input.n, tc.input.assignments, tc.input.element)
